...
public class TeamMembersHandler {
	private final TeamMemberDao dao = new TeamMemberDao();
	...
	public void listByTeam(HttpExchange ex) throws Exception {
		...
		List<TeamMember> list = dao.listByTeam(teamId);
		Router.okJson(ex, Json.stringify(list));
	}

	public void add(HttpExchange ex) throws Exception {
		TeamMember body = Json.parse(Router.body(ex), TeamMember.class);
		if (body == null || isBlank(body.team_id) || isBlank(body.user_id) || isBlank(body.role_id)) {
	...
	public void updateRole(HttpExchange ex) throws Exception {
		...
		if (!roleDao.exists(body.role_id)) { Router.badRequest(ex, "role_id không hợp lệ"); return; }
		boolean ok = dao.updateRole(teamId, userId, body.role_id);
	...
	public void remove(HttpExchange ex) throws Exception {
		Map<String, String> params = Router.pathParams(ex);
		boolean ok = dao.remove(params.get("team_id"), params.get("user_id"));
		if (!ok) { Router.notFound(ex, "Thành viên không tồn tại"); return; }
		Router.noContent(ex);
	}
